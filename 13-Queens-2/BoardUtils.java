...
public class BoardUtils {
    ...
    //
    public static SList<Board> solutions( int n ) {
        return completions( new Board(n) );
    }
    ...
    //
    private static SList<Board> completions( Board b ) {
        int n = b.size();
        int q = b.queensOn();
    
        if ( q == n ) {
            return new SList<Board>(b, new SList<Board>());
        }

        int i = q + 1;
        SList<Board> solutions = new SList<>();
    
        for ( int j=1; j<=n; j=j+1 ) {
            if ( !b.underAttack(i,j) ) {
                solutions = solutions.append(completions(b.addQueen(i,j)));
            }
        }
        return solutions;
    }
